...
def load_emails():
    """Carga los correos del archivo editors_emails.txt"""
    if os.path.exists('editors_emails.txt'):
        with open('editors_emails.txt', 'r') as file:
            return file.read().splitlines()
    else:
        return []

def save_emails(emails):
    """Guarda los correos en el archivo editors_emails.txt"""
    with open('editors_emails.txt', 'w') as file:
        for email in emails:
            file.write(f'{email}\n')
